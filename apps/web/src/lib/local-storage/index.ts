...
export const assignCharacterToLocation = (characterId: string, locationName: string): boolean => {
  try {
    ...
    const filteredLocations = locations.filter(loc => loc.characterIds.length > 0);
...
export const getCharacterLocation = (characterId: string): string | null => {
  const locations = getLocations();
  const location = locations.find(loc => loc.characterIds.includes(characterId));
